...
{%- if cookiecutter.database == "yes" -%}
    public static final ConfigSetting
            ...
            DB_HOSTNAME = new ConfigSetting(config, "MySQL.Hostname",
                    "localhost"),
            DB_PORT = new ConfigSetting(config, "MySQL.Port",
                    3306),
            DB_DATABASE = new ConfigSetting(config, "MySQL.Database",
                    "your-database"),
            DB_USERNAME = new ConfigSetting(config, "MySQL.Username",
                    "user"),
            DB_PASSWORD = new ConfigSetting(config, "MySQL.Password",
                    "pass"),
            DB_USE_SSL = new ConfigSetting(config, "MySQL.Use SSL",
                    false),
            DB_POOL_SIZE = new ConfigSetting(config, "MySQL.Pool Size",
                    3,
